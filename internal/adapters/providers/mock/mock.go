...
//   - vendor：写入 VendorName，对应 domain.Account.Vendor
//   - dims：每次 FetchUsage 回放的维度切片（原样返回，会被 SelectPrimary 处理）
//   - err：FetchUsage 的返回错误；非 nil 时 VendorUsage 仍被填充并计算 Primary
func New(vendor string, dims []domain.UsageDimension, err error) *Provider {
	return &Provider{
		VendorName: vendor,
		Dims:       dims,
		Err:        err,
