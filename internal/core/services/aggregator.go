...
// ErrUnknownVendor 表示账号的 Vendor 在 ProviderLookup 中找不到对应 adapter。
// 用 sentinel + %w 包装，使调用方可 errors.Is 判定，同时消息里携带具体 vendor 名。
var ErrUnknownVendor = errors.New("unknown vendor: no usage provider registered")

// Aggregator 聚合多账号用量拉取：按账号的 Vendor 字段分派到对应 adapter，
// 并发执行且单点失败不连坐（错误只写回对应 VendorUsage.Err，不 panic、不阻断其他账号）。
//
...
//   - 无 adapter：返回填充了账号元信息、Err=ErrUnknownVendor 的 VendorUsage（无 Dimensions、Primary=nil）。
//   - 有 adapter：调用其 FetchUsage；即使返回 err != nil，返回的 VendorUsage 仍被透传
...
func (a *Aggregator) fetchOne(ctx context.Context, acc domain.Account) domain.VendorUsage {
	p, ok := a.lookup.Get(acc.Vendor)
	if !ok {
		// 无 adapter：构造空 u + ErrUnknownVendor（携带 vendor 名便于排障）。
		return domain.VendorUsage{
			AccountID: acc.ID,
			Vendor:    acc.Vendor,
			Label:     acc.Label,
			FetchedAt: time.Now(),
